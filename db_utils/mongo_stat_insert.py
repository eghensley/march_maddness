...
#                
#            old_num += 1
#        for each in db['weighted_%s_%s'% (sa.replace('_','-'), od.replace('_','-'))].find({'_team':teamname.replace(' ','_')}):
#            print(each)
#        latest = 0
#        try:
...
#        except:
#            pass
#        if latest > 0 and db['weighted_%s_%s'% (sa.replace('_','-'), od.replace('_','-'))].find_one({'_team':teamname.replace(' ','_'), '_game': latest})['_date'] == db['weighted_%s_%s'% (sa.replace('_','-'), od.replace('_','-'))].find_one({'_team':teamname.replace(' ','_'), '_game': latest-1})['_date']:
#            print(teamname)
#            print(db['weighted_%s_%s'% (sa.replace('_','-'), od.replace('_','-'))].find_one({'_team':teamname.replace(' ','_'), '_game': latest})['_date'])
#            
        for each in db['weighted_%s_%s'% (sa.replace('_','-'), od.replace('_','-'))].find({'_team':teamname.replace(' ','_')}, sort=[('_game', 1)]):
            if db['weighted_%s_%s'% (sa.replace('_','-'), od.replace('_','-'))].find({'_team':teamname.replace(' ','_'), '_game': each['_game']+1}).count() > 0 and each['_date'] == db['weighted_%s_%s'% (sa.replace('_','-'), od.replace('_','-'))].find_one({'_team':teamname.replace(' ','_'), '_game': each['_game']+1})['_date']:
                print('Date Error: %s %s %s' % (teamname, sa, od))
                print(teamname)
                print(each['_date'])
                db['weighted_%s_%s'% (sa.replace('_','-'), od.replace('_','-'))].delete_many({'_team':teamname.replace(' ','_'), '_game': {'$gte': each['_game']}})
                continue
    return
#           
#            #    return
