...
def keras_preds():
    ...
    for sort in ['keras']:
        ...
        for kind in ['winner', 'ou', 'line']: 
            results = pd.read_csv(os.path.join(output_folder, '%s_results.csv' % (kind)))
            ...
            model = load_model(os.path.join(model_storage, '%s_%s_regression_model.h5' % (kind,sort))) 
            scale = joblib.load(os.path.join(model_storage,  '%s_%s_regression_scaler.pkl' % (kind,sort))) 
            ...
            results.to_csv(os.path.join(output_folder, '%s_results.csv' % (kind)))
