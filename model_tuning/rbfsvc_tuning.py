...
def test_scaler(x, y):
    ...
    for scale in [StandardScaler(), MinMaxScaler(), RobustScaler()]:
        ...
        score = cross_val_score(pipe, x, y, scoring = 'accuracy' ,cv = KFold(n_splits = 3, random_state = 46))
...
def sample_loss_n_feats(parameters):
    ...
    score = cross_val_score(model, x_data[feat_sigs[:feats]], y_data, scoring = 'accuracy' ,cv = KFold(n_splits = 5, random_state = 1108))
...
def sample_loss_c(parameters):
    ...
    score = cross_val_score(model, x_data[feat_sigs[:features]], y_data, scoring = 'accuracy' ,cv = KFold(n_splits = 5, random_state = 88))
    print('----> score: %s' % np.mean(score))
    return np.mean(score)
...
#    sa, od, X_data, Y_data = y_val, x_vals, x_data, y_data
    ...
    model = Pipeline([('scale',scale), ('clf',SVC(random_state = 1108, C = C_, kernel = 'rbf', gamma = g_))])                    
    tune_score = cross_val_score(model, x_data[feat_sigs[:features]], y_data, scoring = 'accuracy' ,cv = KFold(n_splits = 5, random_state = 88))
    print('...Linear SVC Model Finalized')
    tune_score = np.mean(tune_score)
    base_model = Pipeline([('scale',scale), ('clf',GaussianNB())])
    baseline_score = cross_val_score(base_model, x_data[feat_sigs], y_data, scoring = 'accuracy' ,cv = KFold(n_splits = 5, random_state = 88))
