...
for x_vals, y_val in [('pts_allowed', 'ppp_ppp'), ('predictive_pts', 'points')]: # ('pts_scored', 'ppp_ppp'), 
        x_data_stable = pd.read_csv(os.path.join(input_folder, '%s.csv' % (x_vals)))
        x_data_stable = x_data_stable.set_index('Unnamed: 0')
        y_data = np.ravel(x_data_stable[[y_val]])
        x_cols = list(x_data_stable)
        x_cols.remove(y_val)
        if x_vals != 'predictive_pts':
            x_cols.remove(x_vals)
