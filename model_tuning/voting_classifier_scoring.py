...
for sort in ['ou', 'winner', 'line']: 
    acc_voter = cust_classes.VotingClassifier()
    logloss_voter = cust_classes.VotingClassifier()
    combination_voter = cust_classes.VotingClassifier()
    ...
    for kind in ['raw', '+pts']: 
        ...
        for model_name, model_details in saved_models.stored_models[sort][kind].items():
            if os.path.isfile(os.path.join(model_storage, '%s_%s_%s.pkl' % (sort, kind, model_name))):
                ...
                voting_feature = list(set(model_details['features']))
                acc_weight = model_details['acc_weight']
                logloss_weight = model_details['logloss_weight']
                combination_weight = np.mean([model_details['acc_weight'], model_details['logloss_weight']])


                acc_voter.load(model, acc_weight, all_x_data[sort][kind][voting_feature])
                logloss_voter.load(model, logloss_weight, all_x_data[sort][kind][voting_feature])
                combination_voter.load(model, combination_weight, all_x_data[sort][kind][voting_feature])          
...
for df in acc_voter._stored_models.values():
    if len(all_data) == 0:
        all_data = df
    else:
...
model_range = []        
for i in range(int(len(list(all_data))/2)):
    model_range.append(i)
    
idx = []
result = []
for j in range(len(all_data)):
    idx.append(all_data.iloc[j].name)
    one = []
    zero = []
    for each in model_range:
        one.append(all_data.iloc[j].loc['one_'+str(each)])
        zero.append(all_data.iloc[j].loc['zero_'+str(each)])
    result.append([np.sum(zero), np.sum(one)])
